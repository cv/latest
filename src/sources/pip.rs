use super::{extract_version, extract_version_field, Ecosystem, Source};
...
impl Source for PipSource {
    ...
    fn get_version(&self, package: &str) -> Option<String> {
        let pip = ["pip", "pip3"].into_iter()
            .find(|cmd| Command::new("which").arg(cmd).output().map(|o| o.status.success()).unwrap_or(false))?;

        // Try local install first, then PyPI
        Command::new(pip).args(["show", package]).output().ok()
            .filter(|o| o.status.success())
            .and_then(|o| extract_version_field(&String::from_utf8_lossy(&o.stdout)))
            .or_else(|| {
                Command::new(pip).args(["index", "versions", package]).output().ok()
                    .filter(|o| o.status.success())
